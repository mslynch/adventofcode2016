...
pub fn run(filename: Option<&str>) {
    ...
    let file = File::open(filename.unwrap_or("data/day12.txt")).expect("file not found");
    let reader = BufReader::new(file);

    let input: Vec<String> = reader.lines().map(Result::unwrap).collect();
    let instructions = parse_input(&input);
    let part1 = run_and_get_register_a(&instructions);
    println!("part 1: {}", part1);

    let part2 = run_and_get_register_a_part2(&instructions);
    println!("part 2: {}", part2);
}

pub fn run_and_get_register_a(instructions: &[Instruction]) -> i32 {
    let mut memory = Memory::new();
    memory.run_instructions(instructions);
    *memory.registers.get(&'a').unwrap()
}

pub fn run_and_get_register_a_part2(instructions: &[Instruction]) -> i32 {
